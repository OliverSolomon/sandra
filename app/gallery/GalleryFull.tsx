...
export default function GalleryFull() {
  ...
  return (
    <>
      <main className="relative min-h-[100dvh] bg-[#f9f7f4] pb-28 pt-16 md:pt-24">
        <div className="mx-auto max-w-6xl px-6">
          {/* Header */}
          <Reveal className="mb-14 text-center">
            <Link
              href="/#gallery"
              className="mb-8 inline-flex items-center gap-2 font-sans text-xs uppercase tracking-[0.15em] text-stone-400 transition-colors hover:text-stone-600"
            >
            ...
            <h1 className="font-serif text-4xl text-gray-900 md:text-5xl">
              A Life in Photographs
            </h1>
            <div className="mx-auto mt-5 h-px w-12 bg-stone-300" />
            <p className="mx-auto mt-6 max-w-xl font-sans text-sm leading-relaxed text-stone-500">
            ...
            <button
              ...
              className="pressable group mt-8 inline-flex items-center gap-2.5 rounded-full bg-gray-900 py-2.5 pl-5 pr-2.5 font-sans text-sm font-medium tracking-wide text-white transition-colors hover:bg-gray-800"
            >
              Share a Photo
          ...
          {loading ? (
            <div className="columns-2 gap-4 md:columns-3 lg:columns-4 [&>*]:mb-4">
              {[...Array(9)].map((_, i) => (
                <div
                  key={i}
                  className="animate-pulse rounded-lg bg-stone-200/70"
              ...
              {images.map((img, idx) => (
                <Reveal key={img.id} delay={Math.min((idx % 8) * 40, 320)} className="break-inside-avoid">
                  <button
                    ...
                    className="group relative block w-full overflow-hidden rounded-lg bg-stone-100 shadow-[0_1px_2px_rgba(0,0,0,0.04)] ring-1 ring-black/[0.04] transition-shadow duration-500 hover:shadow-[0_14px_44px_-14px_rgba(0,0,0,0.3)]"
                  >
                    <Image
                      src={getUrl(img)}
                      alt={img.alt}
